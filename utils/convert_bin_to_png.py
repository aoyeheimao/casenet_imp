...
def convert_num_to_bitfield(label_data,
                            h,
                            w,
                            root_folder,
                            label_png_name,
    ...
    for n in label_list:  # Iterate in each pixel
        ...
        padded_bit_tensor = torch.cat(
            (torch.zeros(cls_num - actual_len).byte(), bit_tensor.byte()),
            dim=0)
        all_bit_tensor_list.append(padded_bit_tensor)
    all_bit_tensor_list = torch.stack(all_bit_tensor_list).view(h, w, cls_num)
    label_png_dir = os.path.join("label_img", label_png_name.replace(
        '/', '_'))  # eg: label_img/label_img_test_2008_002687
    ...
    for cls_idx in range(cls_num):
        imsave(os.path.join(root_folder, label_png_dir,
                            str(cls_idx) + '.png'),
