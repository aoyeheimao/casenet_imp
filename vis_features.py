...
from modules.CASENet_capital import CASENet_resnet101
...
if __name__ == "__main__":
    ...
    for idx_img in range(len(test_lst)):
        ...
        label_data = torch.stack(label_data).transpose(0, 1).transpose(
            1, 2)  # N X H X W -> H X W X N
        ...
        for i in range(len(score_feats_list)):
            ...
            side = normalized_feature_map(
                feature.data[0][0, :, :].cpu().numpy())
            im = (side * 255).astype(np.uint8)
            if not os.path.exists(
                    os.path.join(args.output_dir, img_base_name_noext)):
                os.makedirs(os.path.join(args.output_dir, img_base_name_noext))
            cv2.imwrite(
                os.path.join(args.output_dir, img_base_name_noext,
                             img_base_name_noext + '_' + feature_str + '.png'),
                im)

        # vis side class activation
        side_cls = normalized_feature_map(
            np.transpose(score_feats5.data[0].cpu().numpy(), (1, 2, 0)))
        for idx_cls in range(num_cls):
            ...
            cv2.imwrite(
                os.path.join(
                    args.output_dir, img_base_name_noext,
                    img_base_name_noext + '_' + 'feats5' + '_' +
                    cls_names[num_cls - idx_cls - 1] + '.png'), im)

        # vis class
        score_output = F.sigmoid(
            score_fuse_feats.transpose(1,
                                       3).transpose(1,
                                                    2)).data[0].cpu().numpy()
        for idx_cls in range(num_cls):
            ...
            plt.imsave(
                os.path.join(
                    args.output_dir, img_base_name_noext, img_base_name_noext +
                    '_fused_pred_' + cls_names[idx_cls] + '.png'), rgb)
        ...
        for idx_cls in range(num_cls):
            ...
            plt.imsave(
                os.path.join(
                    args.output_dir, img_base_name_noext, img_base_name_noext +
                    '_gt_' + cls_names[idx_cls] + '.png'), rgb)
