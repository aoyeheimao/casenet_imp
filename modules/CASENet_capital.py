...
def gen_mapping_layer_name(model):
    ...
    """
    layer_to_name_dict = {
    }  # key is module name in currrent model, value is the filename of numpy
    for (m_name, m) in model.named_parameters():
        if not "res" in m_name:  # This case, name is totally the same.
            if "bn" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.replace(
                        "bn", "scale").replace(".weight", "") + "_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.replace(
                        "bn", "scale").replace(".bias", "") + "_1"
            else:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.replace(".weight",
                                                                "") + "_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.replace(".bias",
                                                                "") + "_1"
        else:
            ...
            label_anno = anno_dict[
                label_idx]  # Represent "a", "b", "c" or "b1", etc

            if "downsample.0" in m_name:  # For the convolution in another branch
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split(
                        '.')[0] + label_anno + "_branch1_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split(
                        '.')[0] + label_anno + "_branch1_1"

            if "downsample.1" in m_name:  # For the BN layer in another branch
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace(
                        "res", "scale") + label_anno + "_branch1_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace(
                        "res", "scale") + label_anno + "_branch1_1"

            if "conv1" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split(
                        '.')[0] + label_anno + "_branch2a_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split(
                        '.')[0] + label_anno + "_branch2a_1"
            if "conv2" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split(
                        '.')[0] + label_anno + "_branch2b_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split(
                        '.')[0] + label_anno + "_branch2b_1"
            if "conv3" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split(
                        '.')[0] + label_anno + "_branch2c_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split(
                        '.')[0] + label_anno + "_branch2c_1"

            # For BN layer
            if "bn1" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace(
                        "res", "scale") + label_anno + "_branch2a_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace(
                        "res", "scale") + label_anno + "_branch2a_1"
            if "bn2" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace(
                        "res", "scale") + label_anno + "_branch2b_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace(
                        "res", "scale") + label_anno + "_branch2b_1"
            if "bn3" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace(
                        "res", "scale") + label_anno + "_branch2c_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace(
                        "res", "scale") + label_anno + "_branch2c_1"
    ...
    for k in layer_to_name_dict.keys():
        if ("bn" in k or "downsample.1" in k) and ("weight" in k):
            ...
            avg_name = [
                layer_to_name_dict[k][:-1].replace("scale", "bn") + "0",
                layer_to_name_dict[k][:-1].replace("scale", "bn") + "2"
            ]
            var_key_name = k.replace(k.split('.')[-1], "running_var")
            var_name = [
                layer_to_name_dict[k][:-1].replace("scale", "bn") + "1",
                layer_to_name_dict[k][:-1].replace("scale", "bn") + "2"
            ]
    ...
    for (m_name, m_data) in model.state_dict().items():
        if m_name in layer_to_name_dict:
            if not ("running_mean" in m_name or "running_var" in m_name):
                npy_path = os.path.join(npy_folder,
                                        layer_to_name_dict[m_name] + ".npy")
                ...
                up_path = os.path.join(npy_folder,
                                       layer_to_name_dict[m_name][0] + ".npy")
                up_data = np.load(open(up_path, 'r'))
                down_path = os.path.join(
                    npy_folder, layer_to_name_dict[m_name][1] + ".npy")
...
class ScaleLayer(nn.Module):
    ...
    def forward(self, input_data):
        return input_data * self.scale.unsqueeze(0).unsqueeze(2).unsqueeze(
            3) + self.bias.unsqueeze(0).unsqueeze(2).unsqueeze(3)
...
#                  block_no,
#                  stride=1,
#                  downsample=None):  # add dilation factor
...
#             layers.append(
#                 block(self.inplanes, planes, special_case=special_case))
...
#             concat_feats
#         )  # BS X 20 X 352 X 352. The output of this will gen loss for this branch. So, totaly 2 loss. (same loss type)
...
# def CASENet_resnet50(num_classes=2):
...
#     if pretrained:
#         utils.load_official_pretrained_model(
#             model, "/ais/gobi5/jiaman/torch/models/resnet101-5d3b4d8f.pth")
#     return model
...
#     feats1, feats2, feats3, feats5, fused_feats = model(input_var,
#                                                         for_vis=True)
