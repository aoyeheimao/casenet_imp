...
class SBDData(data.Dataset):
    def __init__(self, img_folder, label_folder, anno_txt, cls_num,
    ...
    def __getitem__(self, index):
        ...
        tmp_folder = os.path.join("/ais/gobi4/fashion/edge_detection/tmp_npy",
                                  label_name.split('/')[-2],
                                  label_name.split('/')[-1])
        extract_data = zip_file.extract("label", tmp_folder)
        np_data = np.load(os.path.join(tmp_folder,
                                       "label"))  # H X W X NUM_CLASSES

        label_data = []
        for k in range(np_data.shape[2]):
            label_tensor = self.label_transform(
                torch.from_numpy(np_data[:, :, k]).unsqueeze(0).float())
