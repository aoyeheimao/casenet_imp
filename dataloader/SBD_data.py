...
class SBDData(data.Dataset):
    def __init__(self, img_folder, label_folder, anno_txt, hdf5_file_name,
                 input_size, cls_num, img_transform, label_transform):
    ...
    def __getitem__(self, index):
        ...
        np_data = self.h5_f['data/' +
                            label_name.replace('/', '_').replace('bin', 'npy')]

        label_data = []
        num_cls = np_data.shape[2]
        for k in range(num_cls):
            if np_data[:, :, num_cls - 1 - k].sum(
            ) > 0:  # The order is reversed to be consistent with class name idx in official.
                random.seed(
                    seed
                ...
                label_tensor = torch.zeros(1, self.input_size,
